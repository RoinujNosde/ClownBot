  ...
  const flagtr = '🇹🇷';
    ...
    if (reaction.message.channel.id == channel) {
      if (reaction.emoji.name === flagpt) {
          await reaction.message.guild.members.cache.get(user.id).roles.remove(rolept);
      }
      if (reaction.emoji.name === flagru) {
          await reaction.message.guild.members.cache.get(user.id).roles.remove(roleru);
      }
      if (reaction.emoji.name === flagtr) {
          await reaction.message.guild.members.cache.get(user.id).roles.remove(roletr);
      }
    } else {
        return;
    }
